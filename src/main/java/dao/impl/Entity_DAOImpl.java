...
public class Entity_DAOImpl<E> implements Entity_DAO<E> {
	...
	public List<E> getDanhSach(String namedQuery, Class<E> entityType) {
		EntityManager entityManager = entityManagerFactory.createEntityManager();
		try {
			return entityManager.createNamedQuery(namedQuery, entityType).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		entityManager.close();
	...
	public List<E> getDanhSach(Class<E> entityType, Map<String, Object> parameters, int limit, int skip) {
		...
		try {
			...
			return entityManager.createQuery(criteriaBuilderQuery).setMaxResults(limit).setFirstResult(skip)
					.getResultList();
		...
		try {
			...
			if (day != -1) {
				...
				predicates.add(criteriaBuilder
						.equal(criteriaBuilder.function("day", Integer.class, root.get(nameColumn)), day));
			}
			// Thêm điều kiện kiểm tra tháng
			if (month != -1) {
				System.out.println(month);
				predicates.add(criteriaBuilder
						.equal(criteriaBuilder.function("month", Integer.class, root.get(nameColumn)), month));
			}
			// Thêm điều kiện kiểm tra năm
			if (year != -1) {
				System.out.println(year);
				predicates.add(criteriaBuilder
						.equal(criteriaBuilder.function("year", Integer.class, root.get(nameColumn)), year));
			}
	...
	public Long count(String namedQuery, Class<E> entityType) {
		EntityManager entityManager = entityManagerFactory.createEntityManager();
		try {
			return (Long) entityManager.createNamedQuery(namedQuery, entityType).getSingleResult();
		} catch (Exception e) {
			e.printStackTrace();
		}
		entityManager.close();
