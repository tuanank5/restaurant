...
public class DonDatBan_DAOImpl extends Entity_DAOImpl<DonDatBan> implements DonDatBan_DAO {
	...
	public DonDatBan layDonDatMoiNhat() {
		...
		try {
			TypedQuery<DonDatBan> query = em.createQuery("SELECT d FROM DonDatBan d ORDER BY d.ngayGioLapDon DESC",
					DonDatBan.class);
	...
	public List<DonDatBan> timTheoKhachHang(KhachHang kh) {
		EntityManager em = getEntityManager();
		try {
			return em.createQuery("SELECT d FROM DonDatBan d WHERE d.khachHang.maKH = :maKH", DonDatBan.class)
					.setParameter("maKH", kh.getMaKH()).getResultList();
		} finally {
			em.close();
		}
	}
	...
	public List<DonDatBan> timTheoBan(Ban ban) {
		EntityManager em = getEntityManager();
		try {
			return em.createQuery("SELECT d FROM DonDatBan d WHERE d.ban.maBan = :maBan ORDER BY d.ngayGioLapDon DESC",
					DonDatBan.class).setParameter("maBan", ban.getMaBan()).getResultList();
		} finally {
			em.close();
		}
	}
	...
	public Map<String, Integer> countDonDatBanTheoNamNVCuThe(int year, String maNV) {
		...
		try {
			String jpql = "SELECT MONTH(HD.donDatBan.ngayGioLapDon), count(HD.donDatBan.maDatBan) " + "FROM HoaDon HD "
					+ "WHERE YEAR(HD.donDatBan.ngayGioLapDon) = :year AND HD.nhanVien.maNV = :maNV "
					+ "GROUP BY MONTH(HD.donDatBan.ngayGioLapDon) " + "ORDER BY MONTH(HD.donDatBan.ngayGioLapDon)";
			;
	...
	public Map<String, Integer> countDonDatBanTheoNam(int year) {
		...
		Map<String, Integer> demDonDatBan = new HashMap<>();
		try {
			String jpql = "SELECT MONTH(DDB.ngayGioLapDon), count(DDB.maDatBan) " + "FROM DonDatBan DDB "
					+ "WHERE YEAR(DDB.ngayGioLapDon) = :year " + "GROUP BY MONTH(DDB.ngayGioLapDon) "
	...
	public Map<LoaiBan, Integer> countLoaiBanTheoThang(int month, int year) {
		...
		try {
			String jpql = "SELECT B.loaiBan, count(B.loaiBan) " + "FROM DonDatBan DDB " + "JOIN DDB.ban B "
					+ "WHERE MONTH(DDB.ngayGioLapDon) = :month " + "AND YEAR(DDB.ngayGioLapDon) = :year "
					+ "GROUP BY B.loaiBan";
			TypedQuery<Object[]> query = entityManager.createQuery(jpql, Object[].class);
	...
	public Map<LoaiBan, Integer> countLoaiBanTheoNam(int year) {
		...
		try {
			String jpql = "SELECT B.loaiBan, count(B.loaiBan) " + "FROM DonDatBan DDB " + "JOIN DDB.ban B "
					+ "WHERE YEAR(DDB.ngayGioLapDon) = :year " + "GROUP BY B.loaiBan";
	...
	public Map<LoaiBan, Integer> countLoaiBanTheoNgayCuThe(LocalDate dateStart, LocalDate dateEnd) {
		...
		try {
			String jpql = "SELECT B.loaiBan, count(B.loaiBan) " + "FROM DonDatBan DDB " + "JOIN DDB.ban B "
					+ "WHERE DDB.ngayGioLapDon BETWEEN :dateStart AND :dateEnd " + "GROUP BY B.loaiBan";
			TypedQuery<Object[]> query = entityManager.createQuery(jpql, Object[].class);
	...
	public KhachHang getKhachHangTheoMaDatBan(String maDatBan) {
		...
		try {
			TypedQuery<KhachHang> query = em.createQuery(
					"SELECT hd.khachHang FROM HoaDon hd WHERE hd.donDatBan.maDatBan = :maDatBan", KhachHang.class);
	...
	public List<DonDatBan> getAllDDBTruoc() {
		...
		try {
			dsDDB = entityManager
					.createQuery("SELECT HD.donDatBan FROM HoaDon HD WHERE HD.trangThai = :trangThai", DonDatBan.class)
					.setParameter("trangThai", "Đặt trước").getResultList();
