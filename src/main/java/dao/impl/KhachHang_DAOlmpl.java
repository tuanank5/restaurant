...
public class KhachHang_DAOlmpl extends Entity_DAOImpl<KhachHang> implements KhachHang_DAO {
	...
	public KhachHang timTheoSDT(String sdt) {
		...
		try {
			List<KhachHang> ds = em.createQuery("SELECT k FROM KhachHang k WHERE k.sdt = :sdt", KhachHang.class)
	...
	public KhachHang timTheoMa(String maKH) {
		EntityManager em = getEntityManager();
		try {
			List<KhachHang> ds = em.createQuery("SELECT k FROM KhachHang k WHERE k.maKH = :maKH", KhachHang.class)
	...
	public KhachHang getKhachHangTheoSDT(String sdt) {
		...
		try {
			return em.createQuery("SELECT k FROM KhachHang k WHERE k.sdt = :s", KhachHang.class).setParameter("s", sdt)
					.getSingleResult();
		} catch (Exception e) {
			return null;
		} finally {
